...
if st.button("⚡ Run Reconciliation Engine", type="primary"):
    ...
    if books_sales_file and len(gstr1_files) > 0:
        ...
        try:
            ...
            df_sales = ensure_month_column(df_sales) # New Date-to-Month logic!
    ...
    if books_purchase_file and credit_ledger_file:
        ...
        try:
            ...
            # 2. Subtract Debit Notes (If provided)
            if books_dn_file:
            ...
            # 3. Process Electronic Credit Ledger
            ...
            # Separate Ledger into Credit (Availed) and Debit (Utilized)
            # Looks for words starting with "Cr" or "Credit" vs "Dr" or "Debit"
            ...
            for month in all_in_months:
                with st.expander(f"📅 ITC Summary - {month}", expanded=True):
                    ...
                    b_data = b_match.iloc[0] if not b_match.empty else pd.Series({'IGST':0, 'CGST':0, 'SGST':0})
                    
                    c_match = credit_grouped[credit_grouped['Month'] == month]
                    c_data = c_match.iloc[0] if not c_match.empty else pd.Series({'IGST':0, 'CGST':0, 'SGST':0})
                    
                    d_match = debit_grouped[debit_grouped['Month'] == month]
                    d_data = d_match.iloc[0] if not d_match.empty else pd.Series({'IGST':0, 'CGST':0, 'SGST':0})
                    
                    # Core Comparison
                    itc_comparison_df = pd.DataFrame({
                        "Tax Head": ["IGST", "CGST", "SGST"],
                        "ITC as per Books (Net of DN)": [b_data['IGST'], b_data['CGST'], b_data['SGST']],
                        "ITC Availed in Portal (Ledger Cr.)": [c_data['IGST'], c_data['CGST'], c_data['SGST']],
                    })
                    ...
                    # Show Utilized Data at the bottom as requested
                    st.caption("🔻 *Informational: ITC Utilized during this month (from Ledger Dr.)*")
                    util_df = pd.DataFrame({
                        "Tax Head": ["IGST", "CGST", "SGST"],
                        "ITC Utilized": [d_data['IGST'], d_data['CGST'], d_data['SGST']]
                    })
    ...
    # Notice if no files are uploaded
